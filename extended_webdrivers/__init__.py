...
def load_driver(name: str, data: dict) -> ExtendedWebdriver:
    ...
    if name == 'remote':
        ...
        if 'phantomjs' in data and isinstance(data['phantomjs'], dict):
            ...
            if 'desired_capabilities' in data['phantomjs'] and isinstance(
                ...
                desired_capabilities = data['phantomjs']['desired_capabilities']
    ...
    if name in data and isinstance(data[name], dict):
        if 'size' in data[name]:
            ...
            if isinstance(data[name]['size'], (list, tuple)) and len(data[name]['size']) == 2:
            ...
            if isinstance(webdriver_, Chrome) and isinstance(
                    data['chrome']['options']['arguments'], list) and (
                        ...
                        or any('--start-maximized' in x
                            for x in data['chrome']['options']['arguments'])):
                set_window_size = False

            if set_window_size:
                LOGGER.info(
                    f'Setting webdriver window size to {x}x{y}'
                )
        ...
        if 'implicitly_wait' in data[name] and isinstance(data[name]['implicitly_wait'],
        ...
        if 'coordinates' in data[name]:
            latitude = None
            longitude = None
            if isinstance(data[name]['coordinates'], (list, tuple)) and len(data[name]['coordinates']) == 2:
